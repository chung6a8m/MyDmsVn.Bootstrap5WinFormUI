...
public sealed class BootstrapAnimationTests
{
    ...
    {
        Assert.That(() => new BootstrapAnimation(TimeSpan.Zero), Throws.TypeOf<ArgumentOutOfRangeException>());
        Assert.That(() => new BootstrapAnimation(TimeSpan.FromMilliseconds(-1)), Throws.TypeOf<ArgumentOutOfRangeException>());
    }
