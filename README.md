...
The TreeView page demonstrates the native-backed `BootstrapTreeView` contract: semantic selected-node variants, deep hierarchy/scrolling, `FullRowSelect` versus `ShowLines`, checkboxes, native images and state images, caller `ItemHeight`/`NodeFont`, label editing, HotTracking, RTL, drag/drop event observation, explicit handle recreation, and live native hit-test diagnostics. The framework owns `OwnerDrawAll` presentation while native `TreeView` remains authoritative for nodes, selection, expansion, check state, keyboard, editing, hit testing, drag/drop, focus, and accessibility.
...
TreeView is native-backed: `BootstrapTreeView` derives directly from WinForms `TreeView` and adds only the semantic `Variant` public property. The framework owns `OwnerDrawAll` presentation, theme/font/default-row-height lifecycle, vector lines/expanders and framework-rendered checkbox/image/state-image presentation, but native nodes, selection, expansion, check state, label editing, keyboard/focus, HotTracking, hit testing, drag/drop, accessibility, scrolling, and inherited events remain authoritative. It introduces no parallel tree model, selection layer, renderer contract, keyboard engine, drag reorder policy, replacement accessibility provider, or external dependency.
...
Phases 0–16 of the foundation development plan are implemented through release preparation. `BootstrapPagination`, Stage 1 `BootstrapBadge`, Stage 2 `BootstrapAlert`, Stage 3 `BootstrapTooltip`, Stage 4 `BootstrapTabControl`, Stage 5 `BootstrapNumericBox`, Stage 6 `BootstrapComboBox`, Stage 7 `BootstrapDropdown`, Stage 8 `BootstrapToast` / `BootstrapToastContainer`, Stage 9 `BootstrapDatePicker`, the dedicated Select2-style `BootstrapSelect`, the separate `BootstrapCalendar` / `BootstrapCalendarPicker` family, and the native-backed `BootstrapTreeView` are documented compatible control additions on top of that foundation. The current package line remains `1.0.0-rc.1`; promotion to stable `1.0.0` remains gated by the manual release matrix recorded in `docs/RELEASING.md`.
