...
BootstrapSelect pure tests cover logical shell stroke/radius/inset, uniform result-row geometry, hit testing, preferred height, PageUp/PageDown, row-based wheel increments with an exact potentially partial-row final clamp, and popup search-host allocation at 96/120/144/192 DPI. STA tests verify that changing `ResultRowHeight` while open keeps the same popup alive and recomputes bounds, and that DPI changes reapply all presentation metrics before placement using one effective DPI. They also drive Tab and Shift+Tab through the native editor's `PreProcessMessage` path, including both boundaries and nested-container bubbling without wrap. Demo geometry tests measure and draw product secondary text with the same `Graphics` device context and prove the two text rectangles remain contained and non-overlapping at 96/144/192 DPI. Shared-overlay bitmap tests use opaque zero-padding content at 96/120/144/168/192 DPI: composition coverage independently protects all four rounded interior corners, while real `DrawToBitmap` surface rendering verifies that the conservative surface/dropdown window clips retain every blended anti-alias edge pixel without making the silhouette rectangular. The Select page remains the OS-level gate for physical rounded validation/focus strokes, outer/inner popup-border separation, nested `BootstrapTextBox.DeviceDpi`, native text editing/IME, and monitor transitions.
...
Pure tests cover bounded semantic history, effective mutations, newest-first snapshots, strict FIFO under maximum-height pressure, work-area insets with negative coordinates, all four notification-center anchors, and explicit 96/120/144/168/192 per-screen DPI calculations. STA tests cover non-activating per-screen host styles, exact managed/native service-window bounds, first-show `Region` hit/paint bounds (never `TransparencyKey`), transaction-safe history when host transfer fails at capacity, option/renderer snapshots, UI-thread affinity and callback dispatch, topology retirement, reusable `UserClosing` close/hide behavior, uncanceled application/system close reasons, row/keyboard activation, refresh-before-`HistoryChanged` ordering, and deterministic disposal/subscription cleanup on both TFMs.
...
## 15. BootstrapBreadcrumb verification matrix

Pure tests cover item validation and change notification, duplicate/null rejection and collection callback ordering, deterministic segment measurement/arrangement, whole divider-plus-item wrapping, oversized segments, empty dividers, RTL mirroring, and the positive-proposed-width/`MaximumSize.Width`/unbounded preference order.

Bounded STA tests cover empty/one/many-item composition, exact public contract, native `LinkLabel` activation, one event per activation, current item non-actionability, caller-owned navigation, old-generation rejection after structural rebuild, in-place text changes with link identity preservation, RTL/custom/empty dividers, wrapping and repeated layout, enabled state, Light/Dark theme changes, caller font ownership, DPI-scaled metrics, accessible names/roles/focus order, disposal, and theme subscription cleanup. Demo construction and integrated navigation registration are tested on both target frameworks. Any focused raw invocation that can create these controls must retain `--blame-hang --blame-hang-timeout 5m`; the full suite continues to use `./test.ps1`.
