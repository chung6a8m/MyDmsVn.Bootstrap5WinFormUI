...
BootstrapSelect pure tests cover logical shell stroke/radius/inset and popup search-host allocation at 96/120/144/192 DPI. STA tests drive Tab and Shift+Tab through the native editor's `PreProcessMessage` path, including both boundaries and nested-container bubbling without wrap. Shared-overlay bitmap composition tests use opaque zero-padding content and independently sample all four rounded interior corners at 96/120/144/168/192 DPI. The Select page remains the OS-level gate for physical rounded validation/focus strokes, outer/inner popup-border separation, nested `BootstrapTextBox.DeviceDpi`, native text editing/IME, and monitor transitions.
