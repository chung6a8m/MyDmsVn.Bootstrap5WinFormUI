...
`BootstrapRange` derives directly from WinForms `TrackBar` and adds only the semantic `Variant` property. Inherited integer range/value, orientation, tick, RTL, keyboard, mouse, accessibility, and event contracts remain native. The framework uses the verified native custom-draw notification path to theme the rail, thumb, and supported ticks; if a drawing context or safe native geometry is unavailable, or the native tick count exceeds the bounded 2,048-tick custom-draw budget, that part falls back to native painting. V1 intentionally remains a single-thumb integer control with no built-in value tooltip, labels, or progress-filled rail segment.
...
`BootstrapCalendar` and `BootstrapCalendarPicker` are separate framework-owned, owner-drawn date-only controls. `BootstrapCalendarSelectionMode` provides `Single`, inclusive `Range`, and `Multiple` selection; inputs are normalized to `.Date` and bounded by the safe native `DateTimePicker` domain. The calendar projects six weeks by seven days using the current culture’s first day of week and keeps keyboard focus private from public selection state. For summaries, month titles, and calendar accessibility names, the active culture is used unchanged when its calendar can represent the date; otherwise a short-lived clone selects a representable optional Gregorian calendar (then another optional calendar, then invariant formatting). The current culture is never mutated or cached, and its week order, weekday names, and numeric day labels remain authoritative. The picker supplies a formatted/validated collapsed shell and hosts a fresh calendar through native ToolStrip placement/dismissal. This family deliberately does not change the native `BootstrapDatePicker` calendar, localized formatting, keyboard model, or popup ownership.
