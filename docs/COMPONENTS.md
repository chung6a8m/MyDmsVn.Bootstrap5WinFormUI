...
- Hover uses native `HitTest`, stores only the current item index, and never changes selection, focus, checked state, `HotTracking`, `HoverSelection`, or activation. Independently, inherited `HotTracking` retains a distinct underlined presentation when WinForms reports an item as hot.
...
- A focused item retains its native label/item focus cue even when it is not selected. Inherited `DrawItem`, `DrawSubItem`, and `DrawColumnHeader` handlers run before Bootstrap painting; setting `DrawDefault = true` delegates that element to native drawing without Bootstrap pre-painting it.
...
- Caller-owned `SmallImageList`, `LargeImageList`, `StateImageList`, images, fonts, items, columns, and groups are read for presentation but never cloned, resized, replaced, or disposed by the framework. State images and checkbox fallbacks use bounded native probing to stay inside the native state-image hit region in every supported non-Tile view; Details also retains native `IndentCount` geometry without clipping later subitem cells. Header images continue to resolve through `SmallImageList` by index or key. Tile receives no fabricated checkbox, always shows the primary label, maps named headers to named subitems, and applies native column visibility and `DisplayIndex` ordering. Text reading direction follows `RightToLeft` without reversing the caller/native physical `TextAlign`; structural mirroring requires both `RightToLeft = Yes` and `RightToLeftLayout = true`.
