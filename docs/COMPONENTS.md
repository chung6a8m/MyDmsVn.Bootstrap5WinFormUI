...
- Static presentation uses effective opacity `0.5`. Glow uses a cosine ease-in-out cycle over the whole surface, with extrema `0.5 -> 0.2 -> 0.5`. Wave moves a 130-degree local opacity trough across the surface; its sampled mask produces an effective `0.5 -> 0.4 -> 0.5` band over the same base color. Glow and Wave default to a two-second cycle.
...
- The control uses reflected native `NM_CUSTOMDRAW` notifications, verified under real parent HWNDs on both supported target frameworks, to replace only the channel, thumb, and supported tick parts. It validates the notification's `NMHDR` type/source before reading the larger custom-draw payload, because the reflected-notify path also carries header-only notifications. Unknown parts and stages use native default painting. A missing HDC, empty part bounds, unavailable channel geometry, unavailable safe tick positions, or a native tick count above the bounded 2,048-tick custom-draw budget falls back to native painting for that part.
...
- The native custom-draw state flags observed for this control were not reliable enough across the supported runtime/common-controls combinations to own focus/hot/pressed state. The implementation uses normal WinForms focus state plus minimal native-thumb hit-test/capture bookkeeping for hover and pressed presentation. It re-evaluates the last in-control pointer position against current native thumb geometry immediately before thumb painting. Handle destruction clears hot/pressed while retaining the pointer snapshot for recreation; mouse leave, disable, and disposal clear the snapshot, and capture loss clears pressed state.
...
- Hover uses native `HitTest`, stores the current item index and native item rectangle, and invalidates only the previous/current item—including in virtual mode—without changing selection, focus, checked state, `HotTracking`, `HoverSelection`, or activation. Native viewport changes recompute that state under the stationary pointer, including acquiring hover when content moves into a previously blank slot, so mouse-wheel, scrollbar, keyboard, and programmatic scrolling cannot carry or omit the highlight. Independently, inherited `HotTracking` retains a distinct underlined presentation when WinForms reports an item as hot.
- Active selection uses the semantic `Variant`; inactive selection follows `HideSelection`; and `FullRowSelect = false` limits selected treatment to the first label/cell region. The callback's already-hosted `ListViewItem.Selected` state is authoritative in normal and virtual modes, so transient native custom-draw flags cannot visually select unselected grouped rows and virtual painting does not re-enter `RetrieveVirtualItem` through `SelectedIndices`.
- A focused item retains its native label/item focus cue even when it is not selected. Bootstrap and inherited `DrawItem`, `DrawSubItem`, or `DrawColumnHeader` subscribers paint directly to the final HDC, with subscriber painting last, so merely observing an event does not change GDI text fidelity. A reusable, paint-region-sized backup surface grows only when required; keeping `DrawDefault = false` leaves the composed pixels, while setting it to `true` restores the original target pixels and delegates that element to native drawing.
...
- Caller-owned `SmallImageList`, `LargeImageList`, `StateImageList`, images, fonts, items, columns, and groups are read for presentation but never cloned, resized, replaced, or disposed by the framework. State images use bounded native probing to stay inside the native state-image hit region in every view. A caller `StateImageList` always remains authoritative and never falls through to a fabricated checkbox; the no-list checkbox fallback is omitted in Tile. Corresponding Small/Large item images follow the native numeric image position when views change, even when their keys differ. SmallIcon labels retain native leading alignment while LargeIcon labels remain centered and follow `LabelWrap`. Details retains native `IndentCount` geometry without clipping later subitem cells, themes the actual native-header filler in post-paint (including mirrored layout), and keeps header images resolved through `SmallImageList` by index or key. The first transition to Tile synchronizes native Tile layout before painting; Tile receives no fabricated checkbox, always shows the primary label, and projects ordinal subitems `1..N` from the column-header count regardless of header name, width, or display order, with at most 20 projected values. Secondary Tile text is muted only in the neutral state; selection, disabled, and hover states use their contrasting palette foreground unless a genuine per-subitem override applies. Text reading direction follows `RightToLeft` without reversing the caller/native physical `TextAlign`; structural mirroring requires both `RightToLeft = Yes` and `RightToLeftLayout = true`.
- Group membership and view restrictions remain native. Explicit and internal default group headers keep native layout/interaction while their custom-draw foreground, background, separator, task-link, subtitle, and footer colors follow the current Bootstrap theme for readable Light/Dark contrast. Collapsible groups reserve the native affordance slot before task-link presentation. The framework does not fake groups in `View.List`.
...
- V1 excludes a rounded outer border, custom item/data model, loading or empty overlay, sort state/glyphs, replacement group-header layout/interaction, and replacement accessibility.
...
- For an item with `AutoSize = false`, caller-assigned dimensions are tracked independently per axis as minimum layout dimensions rather than being silently replaced by measured content. Constraint-driven resizing from inherited `MinimumSize` or `MaximumSize` does not become a caller-assigned dimension; those constraints still bound the final preferred size in both AutoSize modes. Vertical groups stretch item width to the group's usable width while preserving an explicitly assigned height; horizontal groups preserve explicitly assigned width and height. A group using `AutoSizeMode.GrowOnly` can grow for content but does not shrink its current auto-sized dimensions. Rich-child text, font, padding, anchor, dock, bounds, and visibility changes invalidate the owning layout immediately.
