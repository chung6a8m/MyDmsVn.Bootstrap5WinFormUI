...
- For an item with `AutoSize = false`, caller-assigned dimensions are tracked independently per axis as minimum layout dimensions rather than being silently replaced by measured content. Vertical groups still stretch item width to the group's usable width while preserving an explicitly assigned height; horizontal groups preserve explicitly assigned width and height. Inherited `MinimumSize` and nonzero `MaximumSize` constrain the final preferred size in both AutoSize modes. Rich-child text, font, padding, anchor, dock, bounds, and visibility changes invalidate the owning layout immediately.
