...
- Caller-owned `SmallImageList`, `LargeImageList`, `StateImageList`, images, fonts, items, columns, and groups are read for presentation but never cloned, resized, replaced, or disposed by the framework. State images and checkbox fallbacks use the native state-image hit region in every supported non-Tile view; Details also retains native `IndentCount` geometry. Header images continue to resolve through `SmallImageList` by index or key. Tile receives no fabricated checkbox, always shows the primary label, and selects, hides, and orders additional subitems through the native `Columns` width and `DisplayIndex` contract.
