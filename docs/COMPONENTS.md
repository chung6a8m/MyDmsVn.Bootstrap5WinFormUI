...
- Hover uses native `HitTest`, stores the current item index and native item rectangle, and invalidates only the previous/current item—including in virtual mode—without changing selection, focus, checked state, `HotTracking`, `HoverSelection`, or activation. Native viewport changes recompute that state under the stationary pointer so mouse-wheel, scrollbar, keyboard, and programmatic scrolling cannot carry the highlight with a stale data row. Independently, inherited `HotTracking` retains a distinct underlined presentation when WinForms reports an item as hot.
...
- Group membership and view restrictions remain native. Explicit and internal default group headers keep native layout/interaction while their custom-draw foreground, background, separator, and task-link colors follow the current Bootstrap theme for readable Light/Dark contrast. The framework does not fake groups in `View.List`.
