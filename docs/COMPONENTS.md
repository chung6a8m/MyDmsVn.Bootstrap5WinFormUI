...
- A focused item retains its native label/item focus cue even when it is not selected. Bootstrap presentation and inherited `DrawItem`, `DrawSubItem`, and `DrawColumnHeader` handlers are composed in a reusable WinForms buffer, with subscriber painting last. Keeping `DrawDefault = false` preserves subscriber pixels; setting it to `true` discards the buffer and delegates that element to native drawing without pre-painting the target surface.
...
- Caller-owned `SmallImageList`, `LargeImageList`, `StateImageList`, images, fonts, items, columns, and groups are read for presentation but never cloned, resized, replaced, or disposed by the framework. State images and checkbox fallbacks use bounded native probing to stay inside the native state-image hit region in every supported non-Tile view; Details also retains native `IndentCount` geometry without clipping later subitem cells. Header images continue to resolve through `SmallImageList` by index or key. `LabelWrap` controls LargeIcon multiline labels. Tile receives no fabricated checkbox, always shows the primary label, maps named headers to named subitems, applies native column visibility and `DisplayIndex` ordering, and caps additional projected columns at the native limit of 20. Text reading direction follows `RightToLeft` without reversing the caller/native physical `TextAlign`; structural mirroring requires both `RightToLeft = Yes` and `RightToLeftLayout = true`.
