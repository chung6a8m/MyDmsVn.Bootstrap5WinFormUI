...
- For an item with `AutoSize = false`, caller-assigned dimensions are minimum layout dimensions rather than being silently replaced by measured content. Vertical groups still stretch item width to the group's usable width while preserving the explicit height; horizontal groups preserve both explicit width and height. Rich-child text, font, padding, anchor, dock, bounds, and visibility changes invalidate the owning layout immediately.
